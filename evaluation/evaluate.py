...
def _generate_preds_causal_lm():
        ...
        for batch in tqdm(test_loader, desc="Test"):
            ...
            if config.ANALYSIS:
                ...
                for layer_idx in range(config.NUM_LAYERS):
                    if "causal_attn_gate_analysis" not in batch_stats[layer_idx]:
                        continue
                ...
                kept_ratio = stats_dict["overall"]["kept_ratio"]
                total_kept_ratio_sum += kept_ratio
                total_batches += 1
            else:
    ...
    if config.ANALYSIS:
        ...
        if total_batches > 0:
            ...
            print(f"Prefill kept ratio trung bình: {avg_kept_ratio:.4f}")
    ...
    with torch.inference_mode():
        for batch in tqdm(test_loader, desc="Test"):
            ...
            if config.ANALYSIS:
                ...
                for layer_idx in range(config.NUM_LAYERS):
                    if (
                        "causal_attn_gate_analysis" not in batch_stats[layer_idx]
                        or "non_causal_attn_gate_analysis" not in batch_stats[layer_idx]
                        or "cross_attn_gate_analysis" not in batch_stats[layer_idx]
                    ):
                        continue
                    # Encoder stats
